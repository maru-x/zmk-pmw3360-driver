...
#define PMW3610_REG_DELTA_X_H 0x04
#define PMW3610_REG_DELTA_Y_L 0x05
#define PMW3610_REG_DELTA_Y_H 0x06
#define PMW3610_REG_SQUAL 0x07
#define PMW3610_REG_RAW_DATA_SUM 0x08
#define PMW3610_REG_MAXIMUM_RAW_DATA 0x09
#define PMW3610_REG_MINIMUM_RAW_DATA 0x0A
#define PMW3610_REG_SHUTTER_LOWER 0x0B
#define PMW3610_REG_SHUTTER_UPPER 0x0C
#define PMW3610_REG_CONTROL 0x0D
#define PMW3610_REG_CONFIG1 0x0F
#define PMW3610_REG_CONFIG2 0x10
#define PMW3610_REG_ANGLE_TUNE 0x11
#define PMW3610_REG_FRAME_CAPTURE 0x12
#define PMW3610_REG_SROM_ENABLE 0x13
#define PMW3610_REG_RUN_DOWNSHIFT 0x14
#define PMW3610_REG_REST1_RATE_LOWER 0x15
#define PMW3610_REG_REST1_RATE_UPPER 0x16
#define PMW3610_REG_REST1_DOWNSHIFT 0x17
#define PMW3610_REG_REST2_RATE_LOWER 0x18
#define PMW3610_REG_REST2_RATE_UPPER 0x19
#define PMW3610_REG_REST2_DOWNSHIFT 0x1A
#define PMW3610_REG_REST3_RATE_LOWER 0x1B
#define PMW3610_REG_REST3_RATE_UPPER 0x1C
#define PMW3610_REG_OBSERVATION 0x24
#define PMW3610_REG_DATA_OUT_LOWER 0x25
#define PMW3610_REG_DATA_OUT_UPPER 0x26
#define PMW3610_REG_RAW_DATA_DUMP 0x29
#define PMW3610_REG_SROM_ID 0x2A
#define PMW3610_REG_MIN_SQ_RUN 0x2B
#define PMW3610_REG_RAW_DATA_THRESHOLD 0x2C
#define PMW3610_REG_CONFIG5 0x2F
#define PMW3610_REG_POWER_UP_RESET 0x3A
#define PMW3610_REG_SHUTDOWN 0x3B
#define PMW3610_REG_INVERSE_PRODUCT_ID 0x3F
#define PMW3610_REG_LIFTCUTOFF_TUNE3 0x41
#define PMW3610_REG_ANGLE_SNAP 0x42
#define PMW3610_REG_LIFTCUTOFF_TUNE1 0x4A
#define PMW3610_REG_MOTION_BURST 0x50
#define PMW3610_REG_LIFTCUTOFF_TUNE_TIMEOUT 0x58
#define PMW3610_REG_LIFTCUTOFF_TUNE_MIN_LENGTH 0x5A
#define PMW3610_REG_SROM_LOAD_BURST 0x62
#define PMW3610_REG_LIFT_CONFIG 0x63
#define PMW3610_REG_RAW_DATA_BURST 0x64
#define PMW3610_REG_LIFTCUTOFF_TUNE2 0x65

/* Sensor identification values */
#define PMW3610_PRODUCT_ID 0x42
#define PMW3610_FIRMWARE_ID 0x04

/* Max register count readable in a single motion burst */
#define PMW3610_MAX_BURST_SIZE 12

/* Register count used for reading a single motion burst */
#define PMW3610_BURST_SIZE 6

/* Position of X in motion burst data */
#define PMW3610_DX_POS 2
#define PMW3610_DY_POS 4

/* Rest_En position in Config2 register. */
#define PMW3610_REST_EN_POS 5

#define PMW3610_MAX_CPI 12000
#define PMW3610_MIN_CPI 100
...
#define PMW3610_SVALUE_TO_BOOL(svalue) ((svalue).val1 != 0)

extern const size_t pmw3610_firmware_length;
extern const uint8_t pmw3610_firmware_data[];
...
static int pmw3610_async_init_fw_load_verify(const struct device *dev);
static int pmw3610_async_init_fw_load_continue(const struct device *dev);
static int pmw3610_async_init_fw_load_start(const struct device *dev);

static int (*const async_init_fn[ASYNC_INIT_STEP_COUNT])(
    const struct device *dev) = {
    [ASYNC_INIT_STEP_POWER_UP] = pmw3610_async_init_power_up,
    [ASYNC_INIT_STEP_FW_LOAD_START] = pmw3610_async_init_fw_load_start,
    [ASYNC_INIT_STEP_FW_LOAD_CONTINUE] = pmw3610_async_init_fw_load_continue,
    [ASYNC_INIT_STEP_FW_LOAD_VERIFY] = pmw3610_async_init_fw_load_verify,
...
static int motion_burst_read(const struct device *dev, uint8_t *buf,
  ...
  if (!data->last_read_burst) {
    err = reg_write(dev, PMW3610_REG_MOTION_BURST, 0x00);
    if (err) {
      return err;
    }
  }
...
static int burst_write(const struct device *dev, uint8_t reg,
...
static int update_cpi(const struct device *dev, uint32_t cpi) {
  ...
  int err = reg_write(dev, PMW3610_REG_CONFIG1, value);
  if (err) {
...
static int update_downshift_time(const struct device *dev, uint8_t reg_addr,
  ...
  case PMW3610_REG_RUN_DOWNSHIFT:
    /*
     * Run downshift time = PMW3610_REG_RUN_DOWNSHIFT * 10 ms
     */
    maxtime = 2550;
    mintime = 10;
    break;
...
static int update_sample_time(const struct device *dev, uint8_t reg_addr_lower,
...
static int toggle_rest_modes(const struct device *dev, uint8_t reg_addr,
  ...
  WRITE_BIT(value, PMW3610_REST_EN_POS, enable);
...
static int pmw3610_async_init_fw_load_start(const struct device *dev) {
  ...
  err = reg_write(dev, PMW3610_REG_CONFIG2, 0x00);
  if (err) {
  ...
  err = reg_write(dev, PMW3610_REG_SROM_ENABLE, 0x1D);
  if (err) {
...
static int pmw3610_async_init_fw_load_continue(const struct device *dev) {
  ...
  err = reg_write(dev, PMW3610_REG_SROM_ENABLE, 0x18);
  if (err) {
   ...
   */
  err = burst_write(dev, PMW3610_REG_SROM_LOAD_BURST, pmw3610_firmware_data,
                    pmw3610_firmware_length);
  if (err) {
...
static int pmw3610_async_init_fw_load_verify(const struct device *dev) {
  ...
  uint8_t fw_id;
  err = reg_read(dev, PMW3610_REG_SROM_ID, &fw_id);
  if (err) {
   ...
   */
  err = reg_write(dev, PMW3610_REG_CONFIG2, 0x20);
  if (err) {
...
static void irq_handler(const struct device *gpiob, struct gpio_callback *cb,
  ...
  struct pmw3610_data *data =
      CONTAINER_OF(cb, struct pmw3610_data, irq_gpio_cb);
...
static void trigger_handler(struct k_work *work) {
  ...
  int err = 0;
  struct pmw3610_data *data =
      CONTAINER_OF(work, struct pmw3610_data, trigger_handler_work);
  const struct device *dev = data->dev;
  const struct pmw3610_config *config = dev->config;
...
static int pmw3610_async_init_power_up(const struct device *dev) {
  ...
  return reg_write(dev, PMW3610_REG_POWER_UP_RESET, 0x5A);
}

static int pmw3610_async_init_configure(const struct device *dev) {
  int err;

  err = update_cpi(dev, CONFIG_PMW3610_CPI);

  if (!err) {
    err = update_downshift_time(dev, PMW3610_REG_RUN_DOWNSHIFT,
                                CONFIG_PMW3610_RUN_DOWNSHIFT_TIME_MS);
  }

  if (!err) {
    err = update_downshift_time(dev, PMW3610_REG_REST1_DOWNSHIFT,
                                CONFIG_PMW3610_REST1_DOWNSHIFT_TIME_MS);
  }

  if (!err) {
    err = update_downshift_time(dev, PMW3610_REG_REST2_DOWNSHIFT,
...
static void pmw3610_async_init(struct k_work *work) {
  struct pmw3610_data *data =
      CONTAINER_OF(work, struct pmw3610_data, init_work.work);
  const struct device *dev = data->dev;
...
static int pmw3610_sample_fetch(const struct device *dev,
  ...
  if (!err) {
    int16_t x = sys_get_le16(&buf[PMW3610_DX_POS]);
    int16_t y = sys_get_le16(&buf[PMW3610_DY_POS]);
...
static int pmw3610_channel_get(const struct device *dev,
                               enum sensor_channel chan,
...
static int pmw3610_attr_set(const struct device *dev, enum sensor_channel chan,
  ...
  case PMW3610_ATTR_CPI:
    err = update_cpi(dev, PMW3610_SVALUE_TO_CPI(*val));
    break;

  case PMW3610_ATTR_REST_ENABLE:
    err = toggle_rest_modes(dev, PMW3610_REG_CONFIG2,
                            PMW3610_SVALUE_TO_BOOL(*val));
    break;
  ...
  case PMW3610_ATTR_REST1_SAMPLE_TIME:
    err = update_sample_time(dev, PMW3610_REG_REST1_RATE_LOWER,
                             PMW3610_REG_REST1_RATE_UPPER,
                             PMW3610_SVALUE_TO_TIME(*val));
    break;

  case PMW3610_ATTR_REST2_SAMPLE_TIME:
    err = update_sample_time(dev, PMW3610_REG_REST2_RATE_LOWER,
                             PMW3610_REG_REST2_RATE_UPPER,
                             PMW3610_SVALUE_TO_TIME(*val));
    break;

  case PMW3610_ATTR_REST3_SAMPLE_TIME:
    err = update_sample_time(dev, PMW3610_REG_REST3_RATE_LOWER,
                             PMW3610_REG_REST3_RATE_UPPER,
                             PMW3610_SVALUE_TO_TIME(*val));
    break;
